                ...
                for (int i=0; i<rowValues.length(); i++)
                {
                    ...
                    do
                    {
                        ...
                        if (aOneLine.startsWith("ГОСТ Р"))
                        {
                            ...
                            readGOST(aComponent, aOneLine, aGOSTList, "ГОСТ Р", "ГОСТ Р X [{.X}] -X", aLocation, delimiters);
                        }
                        else
                        if (aOneLine.startsWith("ГОСТ Р МЭК"))
                        {
                            QList<EGOSTDelimiter> delimiters;
                            ...
                            readGOST(aComponent, aOneLine, aGOSTList, "ГОСТ Р МЭК", "ГОСТ Р МЭК X [{.X}] -X-X", aLocation, delimiters);
                        }
                        else
